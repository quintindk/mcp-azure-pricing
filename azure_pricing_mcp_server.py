...
# For stdio, log to stderr; for HTTP/SSE, log to stdout
if settings.TRANSPORT == 'stdio':
    for handler_name, handler_config in logging_config.get('handlers', {}).items():
        if handler_config.get('class') == 'logging.StreamHandler':
            handler_config['stream'] = 'ext://sys.stderr'
...
if settings.MCP_DEBUG:
    ...
    logger.debug(f"DEBUG mode activated ({settings.TRANSPORT})")
...
if __name__ == "__main__":
    ...
    if transport == 'stdio':
        log(f"Starting Azure Pricing MCP Server with stdio transport")
